...
func AddDefaultCollector(name string, sch *Scheduler) error {
	if cInfo, ok := defaultCollectors[name]; ok {
		...
		if err != nil && cInfo.ignoreError == false {
			log.Errorf("Could not add resources metadata provider for %s: %v", name, err)
			return err
		}
		return nil
	}
...
func SetupMetadataCollection(sch *Scheduler, s *serializer.Serializer, hostname string) error {
