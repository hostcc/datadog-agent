...
func TestNormalizeOK(t *testing.T) {
	...
	assert.NoError(t, normalize(ts, s, overridenServiceName))
...
func TestNormalizeNoTraceID(t *testing.T) {
	...
	s.TraceID = 0
	overridenServiceName := ""
	assert.Error(t, normalize(ts, s, overridenServiceName))
...
func TestNormalizeOverrideServiceWhenEmpty(t *testing.T) {
	...
	s := newTestSpan()
	s.Service = ""
	overridenServiceName := "overriden-service-name"
	assert.NoError(t, normalize(ts, s, overridenServiceName))
	assert.Equal(t, "overriden-service-name", s.Service)
	assert.Equal(t, newTagStats(), ts)
}
...
func TestNormalizeInvalidUTF8(t *testing.T) {
	...
	t.Run("service", func(t *testing.T) {
		...
		err := normalize(ts, span, overridenServiceName)
...
func BenchmarkNormalization(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		...
		normalize(ts, span, overridenServiceName)
	}
}
